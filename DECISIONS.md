...
## D-060 — M28 bloom
**Date:** 2026-04-25
**Decision:** Ship bloom as the 18th `PostPass` variant — `PostPass::Bloom(BloomParams { threshold, knee, intensity, radius })` — riding the existing reorderable `PostStack` rather than as a fixed presentation tail. Each bloom slot runs a multi-subpass program against an `Rgba16Float` `BloomPyramid` allocated on `SceneTarget`: bright-pass extract into mip 0, an `N-1` 13-tap Karis-weighted downsample chain, an `N-1` 9-tap tent additive upsample chain (pipeline blend `One+One`), and a replace-blend composite that writes `mix(src, src + bloom * intensity, radius)` into the slot's `dst` (`PostPing`/`PostPong`). The pyramid mip count is `bloom_mip_count_for_size(width, height, render.bloom_max_mips)` with mip 0 starting at half resolution; `bloom_max_mips` is config-validated to `1..=8` with default 6, exposed through `tungsten.json` and the env override `TUNGSTEN_RENDER_BLOOM_MAX_MIPS`. The pyramid is allocated unconditionally on `SceneTarget::new` because the validated range never collapses to zero. Bloom is the first `PostPass` whose slot does not record into a single auto-opened `RenderPass`: the renderer detects the variant before `PassRecorder::begin` and calls `PostStackRenderer::record_bloom_slot`, which opens its own per-subpass passes through the encoder. The four bloom WGSL stages (`bloom_threshold`, `bloom_downsample`, `bloom_upsample`, `bloom_composite`) live under `crates/tungsten-render/src/shaders/stock/`, mirror byte-for-byte under `assets/shaders/stock/`, register in the manifest, and pre-seed the `ShaderModuleCache` at ids `4..=7` (after sprite + SMAA `0..=3`); body-edit hot reload routes through `Renderer::reload_shader` → `BloomPipeline::rebuild_stage_with_module`, with last-known-good fallback on `naga` validation failure. The bloom UBO reuses the engine-wide 256-byte `UniformOverrideBlock` layout: `vec4[0] = inv_src_size`, `vec4[1] = composite_tint`, `f32s = [threshold, knee, intensity, radius]`, `i32s = [mip_count, dst_level, pass_kind, _]`. `bloom_max_mips` is startup-only (relaunch to change), like `msaa`. No HDR `SceneColor` — only the pyramid is `Rgba16Float`. With `PostStack` empty the frame remains byte-identical to the M27 baseline because the pyramid is allocated but never written to or sampled.
**Why:** Bloom is art-direction-shaped: sometimes it must run before tone-mapping (the HDR-style stack), sometimes after (cheap stylized stack with film grain on top). That requirement makes it a `PostPass`, not a fixed tail like SMAA from `D-059`. Multi-subpass routing through the encoder is the smallest deviation from the per-slot single-render-pass model that still expresses a mip-chain algorithm; lifting all post slots to encoder-level recording would have been a wider refactor with no payoff for the 17 single-pass effects. Starting mip 0 at half resolution keeps the default 6-mip pyramid near one-third the cost of a full-res `Rgba16Float` target (≈ 5.5 MiB at 1080p, ≈ 22 MiB at 4K) — acceptable for desktop M28 and the COD/Frostbite convention. The `Rgba16Float` pyramid avoids 8-bit sRGB quantization through downsample/upsample but does not require an HDR scene attachment, so `D-058`'s "no HDR `SceneColor` in M26" stays in force; M33+ may revisit when emissives outgrow the LDR ceiling. Karis 1/(1+luma) weighting on the 13-tap downsample suppresses single-pixel fireflies without changing the public API. The 256-byte UBO contract from `D-058`/`D-059` already accommodates the per-frame knobs, so no new layout enters the engine. Encoding `bloom_max_mips` as startup-only avoids a runtime pyramid-reallocation seam that has no caller demand at this milestone (the same reasoning as `msaa`); a future runtime knob can land via the request/apply pattern from `D-043`/`D-059` when needed. No new runtime dependency: `wgpu`, `bytemuck`, and `glam` already cover the surface (`D-015` satisfied via existing rule citations).
**Consequences:** Extends `D-053` with four new body-edit reloadable shader rows (`bloom_threshold`, `bloom_downsample`, `bloom_upsample`, `bloom_composite`); the LUT/UBO conventions and `naga` last-known-good behaviour from `D-057`/`D-059` carry over verbatim. Narrows neither `D-058` nor `D-059`: bloom is a reorderable `PostPass`, not a fixed presentation tail, and `SceneColor` stays sRGB. `PostPass` grows to 18 variants and remains a closed enum — adding the next variant is the same four-point change (variant + pipeline + stock WGSL + roster row) noted in `D-054`. `BloomPipeline` is the first occupant of an encoder-level slot path; if a second multi-subpass `PostPass` appears later the renderer's slot dispatch should be lifted into a shared trait or enum, but M28 keeps the routing inline and flags the precedent in `render_frame_internal`. Two `PostPass::Bloom` slots in the same stack are legal and double-apply; each invocation reuses the same pyramid and overwrites mip levels mid-frame — documented as user responsibility, not guarded against. `BloomParams` is added to `tungsten-core::post` and re-exported through `lib.rs`. `examples/04_shader_playground` gains the `KeyL` toggle, `Y/H U/J I/K` live-tune actions, an emissive-quad sprite for the LDR demo fixture, and the `TUNGSTEN_BLOOM_FIXTURE` + `TUNGSTEN_POST_STACK_FIXTURE=bloom_only` env pins. The smoke matrix gains a bloom row; the showcase capture and regen recipe land alongside `docs/showcase/`.

## D-061 — M29 2D forward normal-mapped lighting
**Date:** 2026-04-25
**Decision:** Ship 2D forward lighting as a sibling pipeline inside the scene pass. `Light { kind, color, intensity }` and a closed `LightKind::{ Point, Directional }` enum live in `tungsten-core::components`; `AmbientLight(Vec3)` is a world resource defaulting to `Vec3::ONE`. Render-side `LightingResources` owns one 544-byte `LightUbo` (16 lights × 32 bytes + `vec4<u32>` count_pad + `vec4<f32>` ambient) bound at group 2 of a new `LitSpritePipeline`; the pipeline reuses the existing sprite vertex/instance layout and binds a parallel albedo + normal + emissive bundle at group 1 (three texture views + one filtering sampler). Sprites carrying optional `normal_map` / `emissive_mask` siblings in the manifest pack into parallel atlas canvases keyed by the same packed rect — no second packer call, no separate sprite ids, no UV plumbing. `extract_sprites_default` flips `SpriteBatch.lit` on the resolved `SpriteAsset.lit_atlas.is_some()` axis; lit batches force `material_id = None` and warn when a sprite carries both (lit wins, M29 explicit non-goal). `extract_lights` runs every frame, culls by camera-AABB squared-distance, retains directionals first, caps at `LIGHT_CAP = 16`. The four lighting WGSL files (`lit_sprite`, `emissive_mask`, `rim_light`) live under `assets/shaders/` (lit) and `assets/shaders/stock/` (helpers), mirror byte-for-byte to `crates/tungsten-render/src/shaders/stock/`, register in the manifest, and pre-seed the `ShaderModuleCache` at ids `8..=10` (after sprite + SMAA + bloom). Body-edit hot reload routes through `Renderer::reload_shader` → `LitSpritePipeline::rebuild_with_shader` for the lit shader; the helpers are validated only (no pipeline behind them). Normal textures upload as linear `Rgba8Unorm` so tangent-space vectors are not gamma-decoded; emissive uploads premultiplied alpha-as-RGB so single-channel masks render consistently. With no lit sprites and an empty light list the captured frame stays byte-identical to the M28 baseline — the unlit pipeline is unchanged and the lit pipeline never runs.
**Why:** Sibling files share UV with the albedo for free, so the lit pool keys off the existing `TextureHandle` and the extract layer needs only one new boolean (lit) — strictly less code than parallel sprite ids would require, and it keeps `D-009`'s ID-only seam intact. The lit pipeline is a sibling rather than a `material_id` extension because lit shading is a renderer-owned global (per-frame UBO, not per-entity), so squeezing it into the per-material `UniformOverrideBlock` would force every material author into the lighting model. Forward shading with a 16-light cap fits 2D scenes (UBO size clamps at 544 bytes, comfortably under wgpu's 16 KiB minimum spec) and avoids dragging a deferred path into Phase 4. `Directional` lights always retained because losing them under heavy point-light pressure dims the entire scene noticeably, while losing the farthest point light is locally invisible. Tangent-space normals interpreted as world-axis is the conventional 2D shortcut: there is no per-fragment tangent frame for a sprite plane, so `(n.xy, n.z)` is read directly. Closed-enum `LightKind` mirrors `Easing` (`D-054` precedent): adding a spotlight is an explicit plan-file change, not a silent runtime registration. No new runtime dependency: `wgpu`, `bytemuck`, and `glam` already cover the surface.
**Consequences:** `SpriteAsset` grows optional sibling paths plus a `lit_atlas: Option<TextureHandle>` marker; `SpriteEntry` grows two optional sibling string fields; `extract_sprites_default` grows a `lit` keying axis (still byte-equal output when no sprite carries aux atlases). New shader id triple (`lit_sprite`, `emissive_mask`, `rim_light`) extends `D-053`'s body-edit reload table. Lit + material composition is intentionally out-of-scope: a sprite carrying both warns and uses lit; rotating a lit sprite is a documented M33 follow-up (the 2D shader does not rotate the sampled normal). Narrows neither `D-058` nor `D-060`: emissive bright pixels feed the existing M28 bloom thresholding without additional wiring; `SceneColor` stays sRGB. The platformer example gains four `walk_*_n.png` and four `walk_*_e.png` siblings, an `orbit_lights_system`, and a `TUNGSTEN_LIGHTING_FIXTURE=on|off` env switch that gates light spawn + lit-batch routing on the player. The smoke matrix gains a lighting row; the showcase capture (`docs/showcase/lighting_off_vs_on.png`) and regen recipe land alongside the M27/M28 entries.
