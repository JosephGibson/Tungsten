...
/// Particle config load/validation error.
#[derive(Debug, Error)]
...
pub enum EmissionKind {
    ...
    /// Sustained particles per second.
...
pub enum InitialVelocity {
    ...
    /// Random full-circle direction.
    Radial { speed: Range },
    /// Normalized direction scaled by `speed`.
    Vector { direction: [f32; 2], speed: Range },
}

/// CPU-side blend hint before shared sprite pipeline.
...
/// Piecewise-linear curve over `t in [0, 1]`.
#[derive(Debug, Clone)]
...
pub struct ParticleConfig {
    ...
    /// Spawn-time RGBA multiplier.
    #[serde(default = "default_white_rgba")]
    pub tint: [f32; 4],
}

impl ParticleConfig {
    /// Load and validate particle config from disk.
...
/// D-014 global particle cap resource.
...
/// Particle config registry; `Arc` swap affects future spawns only.
...
impl ParticleConfigRegistry {
    ...
    /// Register config and mint fresh `AssetId`.
    pub fn register(
        &mut self,
        name: String,
        path: PathBuf,
    ...
    /// Replace config `Arc` without changing `AssetId`.
