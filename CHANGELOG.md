...
- **New example — `example-04-scene-state`:** end-to-end demo of the `MainMenu → Gameplay → Pause → Gameplay` flow. Gameplay entities come from `scene.json` via `spawn_scene`; Pause overlays Gameplay without tearing the scene down; the HUD `state` row mirrors the active state id.
